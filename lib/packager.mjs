import { readFileSync, statSync, rmSync, unlinkSync, existsSync } from 'fs';
...
export async function packageAndUpload(projectDir, projectId, { keepLocal = false } = {}) {
  ...
  try {
    ...
    if (!keepLocal) {
      try {
        ...
        console.log(`  ${tag.build} ${neon.dim('Cleaned up local build files')}`);
      } catch (cleanupErr) {
        console.log(`  ${tag.build} ${neon.dim('Warning: cleanup failed — ' + cleanupErr.message)}`);
      }
    } else {
    ...
    // Clean up ZIP on failure (keep project dir for potential retry)
    try { if (existsSync(zipPath)) unlinkSync(zipPath); } catch {}
